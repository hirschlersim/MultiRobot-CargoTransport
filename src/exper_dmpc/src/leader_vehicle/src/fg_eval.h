...
namespace {
	...
	:N_(N_),m_(m_),p_(p_),xr_(xr_),yr_(yr_),thetar_(thetar_),d_(d_),xinit_(xinit_),yinit_(yinit_),thetainit_(thetainit_),ts_(ts_),safety_dist_(safety_dist_),obst_(obst_),neig_(neig_){
        ...
        {   assert( fg.size() ==   (m_ + p_ -1)*(3*N_+3) + 6*N_ + 6+ 1);
...
**/
	...
	for(size_t j = 0; j < N_+1;j++){
		...
		//dist err term
		fg[0] += 0.2*((v_states[j*5] - xr_)*(v_states[j*5] - xr_)+(v_states[j*5+1] - yr_)*(v_states[j*5+1] - yr_));

		//speed err term
		fg[0] += v_states[j*5+3] * 0.4 * v_states[j*5+3] + v_states[j*5+4] * 0.4 * v_states[j*5+4];

		//angle err term
		CppAD::AD<double> x_err = abs(v_states[j*5] - xr_);
		CppAD::AD<double> y_err = abs(v_states[j*5+1] - yr_);
		CppAD::AD<double> dist_err = 0.0005 / ((x_err + y_err)*(x_err + y_err) + 0.000000001); //catch div by 0
		fg[0] += (v_states[j*5+2] - thetar_) * dist_err * (v_states[j*5+2] - thetar_);
	}
