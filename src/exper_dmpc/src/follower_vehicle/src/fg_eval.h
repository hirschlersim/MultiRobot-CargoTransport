...
**/
	...
	for(size_t j = 0; j < N_+1;j++){
		...
		CppAD::AD<double> err_angle = 1;
		CppAD::AD<double> err_dist = 15;
		CppAD::AD<double> err_speed = 0.35;

		//dist err term
		fg[0] += err_dist*((v_states[j*5] - xr_)*(v_states[j*5] - xr_)+(v_states[j*5+1] - yr_)*(v_states[j*5+1] - yr_));

		//speed err term
		fg[0] += v_states[j*5+3] * err_speed * v_states[j*5+3] + v_states[j*5+4] * err_speed * v_states[j*5+4];

		//angle err term
		fg[0] += (v_states[j*5+2] - thetar_) * err_angle * (v_states[j*5+2] - thetar_);
	}
